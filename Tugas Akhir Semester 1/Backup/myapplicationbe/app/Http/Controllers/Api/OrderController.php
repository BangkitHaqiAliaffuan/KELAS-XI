...
class OrderController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        DB::beginTransaction();
        try {
            // Improvement: Hindari race condition dengan lockForUpdate()
            // Baris ini akan mengunci row di DB sampai transaksi commit/rollback.
            $listing = MarketplaceListing::active()->lockForUpdate()->find($validated['listing_id']);

            if (!$listing) {
                DB::rollBack();
                return response()->json(['message' => 'Listing tidak ditemukan.'], 404);
            }
